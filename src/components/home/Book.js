...
export default function Book() {
  ...
  const [availableRoomsInfo, setAvailableRoomsInfo] = useState([]);
  ...
  const handleCheckInChange = (date) => {
    ...
    try {
      ...
      const info = availableIds.map((id) => getRoomWithAvailability(id, iso));
      setAvailableRoomsInfo(info);
      setAvailabilityMessage(`${info.length} room category(ies) available for ${iso}`);
    } catch (err) {
                            ...
                            <div>
                              ...
                              <div style={{ display: "flex", gap: "15px", marginBottom: "15px", flexWrap: "wrap" }}>
                                <span style={{ fontSize: "14px", color: "#666" }}>
                                  ...
                                  Max {roomCapacities[room.id] || 2} Guest{roomCapacities[room.id] > 1 ? "s" : ""}
                                </span>
                                <span style={{ fontSize: "14px", color: "#666" }}>
                  ...
                  <div className="mt-5 pt-4" style={{ borderTop: "1px solid #e0e0e0" }}>
                    ...
                    <div className="row g-3">
                      {roomSelections.map((sel, idx) => {
                        ...
                        const options = availableRoomsInfo.filter(
                          (info) =>
                            roomCapacities[info.id] >= minimalPerRoom &&
